...
public class SeasonsConfig {
    ...
    public int shoreBreakCoastSearchRadius() {
        return Math.max(4, plugin.getConfig().getInt("waves.shore-break.coast-search-radius", 40));
    }
    ...
    public double shoreBreakMaxInlandDistance() {
        return Math.max(1, plugin.getConfig().getDouble("waves.shore-break.max-inland-distance", 10.0));
    }

    public double shoreBreakAdvanceSpeed() {
        return Math.max(0.1, plugin.getConfig().getDouble("waves.shore-break.advance-speed", 1.0));
    }
