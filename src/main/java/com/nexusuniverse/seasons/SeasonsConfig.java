...
public class SeasonsConfig {
    ...
    /** Ground push multiplier for a completely full inventory (all 36 storage slots occupied) -- lower than 1.0 means a heavily-loaded player is harder to push around, as if the extra weight anchors them. */
    public double windGroundPushMultiplierFull() {
        return Math.max(0.0, plugin.getConfig().getDouble("wind.inventory-weight.ground-push-multiplier-full", 0.5));
    }
    ...
    /** How many cobweb points make up each ring of the funnel's shape -- kept lower than the particle ring's own point count on purpose, since a real block is much more visually "solid" per-point than a particle is. */
    public int tornadoCobwebRingPointCount() {
        return Math.max(3, plugin.getConfig().getInt("tornado.cobweb-funnel.ring-point-count", 8));
    }
    ...
    public int tornadoCobwebMaxBlocksPerUpdate() {
        return Math.max(1, plugin.getConfig().getInt("tornado.cobweb-funnel.max-blocks-per-update", 200));
    }
    ...
    public int tornadoCobwebWallThickness() {
        return Math.max(1, plugin.getConfig().getInt("tornado.cobweb-funnel.wall-thickness", 5));
    }

    /** Whether a sparse set of real cobweb blocks also gets scattered through the particle cloud canopy's own log-spiral shape, for a bit of solid depth mixed into what's otherwise all particles up there. */
    ...
    /** How many cobweb accent points exist in the canopy at once -- deliberately small; this is meant as a depth accent, not a full solid canopy (the particles already handle that density). */
    public int tornadoCobwebCanopyAccentCount() {
        return Math.max(0, plugin.getConfig().getInt("tornado.cobweb-funnel.canopy-accents.count", 18));
    }
    ...
    /** Particles spawned for the canopy every tick -- this is a wide area, so it needs real density to not look sparse/patchy. */
    public int tornadoCanopyDensity() {
        return Math.max(0, plugin.getConfig().getInt("tornado.canopy.density", 130));
    }
    ...
    public double tornadoDebrisSpawnChancePerTick() {
        return Math.max(0.0, plugin.getConfig().getDouble("tornado.debris.spawn-chance-per-tick", 0.35));
    }
    ...
    public int tornadoDebrisMaxActive() {
        return Math.max(0, plugin.getConfig().getInt("tornado.debris.max-active", 40));
    }

    /** How long (ticks) one debris block spirals before it's removed -- ~5 real seconds by default. */
    public int tornadoDebrisLifetimeTicks() {
        return Math.max(1, plugin.getConfig().getInt("tornado.debris.lifetime-ticks", 100));
    }
