...
public class SeasonsConfig {
    ...
    /**
     * How many extra dense, swirling white-smoke "body fill" particles get scattered inside the
     * funnel's tapered radius every tick, on top of the existing spinning ring structure -- the
     ...
     * fill and falls back to the plain ring-only look from before.
     */
    public int tornadoFunnelFillDensity() {
        return Math.max(0, plugin.getConfig().getInt("tornado.funnel-fill-density", 40));
    }
