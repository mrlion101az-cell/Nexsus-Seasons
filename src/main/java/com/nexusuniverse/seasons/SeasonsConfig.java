...
public class SeasonsConfig {
    ...
    public int tornadoCobwebRingPointCount() {
        return Math.max(3, plugin.getConfig().getInt("tornado.cobweb-funnel.ring-point-count", 12));
    }
    ...
    public int tornadoCobwebMaxBlocksPerUpdate() {
        return Math.max(1, plugin.getConfig().getInt("tornado.cobweb-funnel.max-blocks-per-update", 350));
    }
    ...
    public int tornadoCobwebWallThickness() {
        return Math.max(1, plugin.getConfig().getInt("tornado.cobweb-funnel.wall-thickness", 7));
    }
    ...
    /** How many cobweb accent points exist in the canopy at once -- roughly half of tornado.canopy.density's particle count by default, for a genuine ~50/50 particles-vs-solid-blocks mix up there rather than just a light accent. */
    public int tornadoCobwebCanopyAccentCount() {
        return Math.max(0, plugin.getConfig().getInt("tornado.cobweb-funnel.canopy-accents.count", 65));
    }
