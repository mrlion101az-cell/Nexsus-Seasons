...
public class TsunamiManager {
    ...
    public boolean spawnNear(Location location) {
        ...
        if (!engine.findCoastNear(location, config.tsunamiCoastSearchRadius(), location.getWorld().getSeaLevel())) return false;
    ...
    private void tick() {
        ...
        if (engine.isActive()) {
            ...
            if (finished && config.waterLevelingEnabled() && engine.coastAnchor() != null) {
                WaterLevelingEngine.levelArea(engine.coastAnchor(), config.waterLevelingRadius(), config.waterLevelingMaxBlocksPerPass());
            }
