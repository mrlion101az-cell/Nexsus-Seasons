...
public class TsunamiManager {
    ...
    public boolean spawnNear(Location location) {
        ...
        if (!engine.findCoastNear(location, config.tsunamiCoastSearchRadius())) return false;
    ...
    private void tick() {
        ...
        if (engine.isActive()) engine.tick(); // no-op during the pending warning countdown -- the engine hasn't actually started yet
    }
