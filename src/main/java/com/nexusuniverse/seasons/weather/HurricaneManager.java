...
public class HurricaneManager {
    ...
    private void tick() {
        if (!active) {
            ...
            if (surge.isActive()) surge.tick(); // let a pulse that was still draining when the storm ended finish naturally
            return;
        }
    ...
    private void tickStormSurge(double intensity) {
        ...
        if (!surge.findCoastNear(anchor.getLocation(), config.hurricaneSurgeCoastSearchRadius())) return;
