...
public class HurricaneManager {
    ...
    public void forceStart(int durationMinutes) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            player.sendMessage("§4§lA hurricane is building.");
        }
    }
        ...
        if (!active) {
            ...
            if (surge.isActive()) {
                ...
                if (finished && config.waterLevelingEnabled() && surge.coastAnchor() != null) {
                    WaterLevelingEngine.levelArea(surge.coastAnchor(), config.waterLevelingRadius(), config.waterLevelingMaxBlocksPerPass());
                }
            }
            return;
        }
    ...
    private void tickStormSurge(double intensity) {
        ...
        if (!surge.findCoastNear(anchor.getLocation(), config.hurricaneSurgeCoastSearchRadius(), anchor.getWorld().getSeaLevel())) return;
