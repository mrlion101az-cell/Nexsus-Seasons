 ...
 * particle rings (funnelRadiusAt() -- pointed/narrow at the base, flared/wide at the top, a real
 * funnel-cloud silhouette, NOT narrow-at-both-ends) that give it a defined spinning shape, PLUS a
 * dense white-smoke/cloud body fill (renderDenseBody(), the same dense-random-scatter-in-a-radius
 * technique FogManager/BlizzardManager use for their whiteout look) that thickens it into a real,
 * solid-looking cyclone rather than a sparse wireframe. BOTH layers spawn every particle with the
 * spawnParticle(count=0, offsets-as-velocity) trick -- each individual particle gets real
