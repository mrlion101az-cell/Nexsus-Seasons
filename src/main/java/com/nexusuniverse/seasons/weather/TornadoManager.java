 ...
 * look) that thickens it into a real, solid-looking cyclone rather than a sparse wireframe --
 * every fill particle also gets genuine outward-swirling velocity via the spawnParticle(count=0,
 * ...) trick, so the density itself visibly spirals rather than just sitting there. "Leaf debris"
 * is simulated with tinted DUST particles (green/brown) rather than a literal leaf particle,
 * since there isn't a stable, guaranteed-available vanilla particle for that -- see FogManager's
 * doc comment for the same kind of honest caveat applied to real API limits.
 *
