 ...
 * -- no resource pack, no custom models. The funnel is TWO layers together: a stack of rotating
 * particle rings (wider at the top and bottom, narrower through the middle) that give it a
 * defined spinning silhouette, PLUS a dense white-smoke body fill (renderDenseBody(), the same
 * dense-random-scatter-in-a-radius technique FogManager/BlizzardManager use for their whiteout
 * look) that thickens it into a real, solid-looking cyclone rather than a sparse wireframe. BOTH
 * layers spawn every particle with the spawnParticle(count=0, offsets-as-velocity) trick -- each
 * individual particle gets real tangential motion curving around the funnel's own center, not
 * just a new randomly-jittered position every tick. That distinction matters: recomputing a
 * particle's position at a rotated angle each frame LOOKS like a rotating shape from a distance,
 * but each individual particle isn't actually moving -- giving every particle its own curving
 * velocity is what makes it read as something genuinely spinning up close. "Leaf debris" is
 * simulated with tinted DUST particles (green/brown) rather than a literal leaf particle, since
 * there isn't a stable, guaranteed-available vanilla particle for that -- see FogManager's doc
 * comment for the same kind of honest caveat applied to real API limits.
 *
