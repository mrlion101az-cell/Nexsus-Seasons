...
public class NexusSeasonsPlugin extends JavaPlugin implements NexusSeasonsAPI {
    ...
    public void onEnable() {
        ...
        // independent of which day/night mode is active above -- also protects vanilla's own
        // cycle (and weather) when day-night.enabled is off, not just the custom cycle
        this.cycleLock = new CycleLockManager(this, config);
        cycleLock.start();
        getServer().getPluginManager().registerEvents(new CycleLockGuard(config, getLogger()), this);
